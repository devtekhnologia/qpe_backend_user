...
import Subject, { Data } from "./subjectModel";
...
export const subjectService = {
    ...
    getSubject: async (reg_id: string) => {
        const result = await Subject.find({ reg_id: reg_id, status: 1 }).sort({ _id: -1 }); // Filter by institute_id
